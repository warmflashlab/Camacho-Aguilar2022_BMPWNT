...
nuc2nucMeans(iCon,:) = nuc2nucMeans(iCon,:)./control;
end
% plot nuc2nucMeans
figure; clf; hold on;
for iCon = 1:analysisParam.nCon;
plot(analysisParam.plotX(1:size(nuc2nucMeans,2)),nuc2nucMeans(iCon,:)./mean(nuc2nucMeans(iCon,preLigand)),'Color',colors(iCon,:),'LineWidth',lineWidth);
end
...
title('mean signaling, divide baseline, subtract preligand');
savefig('figures/Gfp2RfpDivBaselineSubPreligand.fig');
