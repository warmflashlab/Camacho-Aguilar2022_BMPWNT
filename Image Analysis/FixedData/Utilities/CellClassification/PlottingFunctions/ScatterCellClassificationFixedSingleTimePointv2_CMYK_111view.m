function ScatterCellClassificationFixedSingleTimePointv2_CMYK_111view(samplenumber,conditionnum,blackbackground,bigplot) 
...
figure;
if bigplot
set(gcf,'Position',[00 00 900 900])
else
...
%         end
        ...
        if ndimensions == 3
            ...
            scatter3(DataPlot(:,2),DataPlot(:,1),DataPlot(:,3),pointsize,MatrixColors(singleCellsClassified{analysisParam.ConditionOrder(conditionnum)}(:,end),:),'filled','MarkerEdgeAlpha',0.5,'MarkerFaceAlpha',0.5)%,'MarkerEdgeColor',[0.5,0.5,0.5])
            
%             xlabel(analysisParam.Channels{analysisParam.ChannelOrder3D(2)},'Color',lineandtextcolor);
%             ylabel(analysisParam.Channels{analysisParam.ChannelOrder3D(1)},'Color',lineandtextcolor);
%             zlabel(analysisParam.Channels{analysisParam.ChannelOrder3D(3)},'Color',lineandtextcolor);
            xlim([minlim(2),maxlim(2)])
            ylim([minlim(1),maxlim(1)])
            zlim([minlim(3),maxlim(3)])
            
            xticks(round(linspace(minlim(2),maxlim(2),4),1))
            yticks(round(linspace(minlim(1),maxlim(1),4),1))
