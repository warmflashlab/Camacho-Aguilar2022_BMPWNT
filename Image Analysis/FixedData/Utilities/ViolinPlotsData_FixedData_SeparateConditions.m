...
function ViolinPlotsData_FixedData_SeparateConditions(blackBG)
...
for condition = 1:analysisParam.nCon
        ...
        AllDataConditions{condition} = AllDataMatrixDAPInorm{condition}(:,3+analysisParam.ChannelOrder);
        
end

Conditionsname = analysisParam.conNamesPlotsamples;

%%

maxlim = quantile(AllDataConditions{1},0.9995);
minlim = quantile(AllDataConditions{1},0.0005);

for conditionnum=2:analysisParam.nCon
        
        DataPlot = AllDataConditions{conditionnum};
        
        maxlim = max([maxlim;quantile(DataPlot,0.9995)]);
        minlim = min([minlim;quantile(DataPlot,0.0005)]);
...
for subsamples=1:analysisParam.numseparateconditions
for channelnum = 1:analysisParam.nChannels

subplot(analysisParam.nChannels,analysisParam.numseparateconditions,subsamples+2*(channelnum-1))
...
legendhandle = {};
    ...
    for daynum = 1:length(analysisParam.samplesconditions{subsamples})
...
%         DataPlot = ((AllDataConditions{conditionnum}{daynum}(:,2)./AllDataConditions{conditionnum}{daynum}(:,1))/normalizer(conditionnum))./(control(times(daynum)));
        DataPlot = AllDataConditions{analysisParam.samplesconditions{subsamples}(daynum)}(:,channelnum);
...
%         scatter(daytick, mean(DataPlot),10,'filled','MarkerEdgeColor',colourclusters{channelnum},'MarkerFaceColor',colourclusters{channelnum},'LineWidth',0.1)
        ...
        daystickslabels{daynum} = Conditionsname{analysisParam.samplesconditions{subsamples}(daynum)};
...
%     title([genesnames{1}{genenum},' dynamics'],'fontsize',10)
    ...
    if channelnum<analysisParam.nChannels
        xticklabels((cell(1,length(analysisParam.samplesconditions{subsamples}))));
    else
    xticklabels((Conditionsname(analysisParam.samplesconditions{subsamples})));
    
    end
    
    if channelnum==1
        
        title(analysisParam.titlesamples{subsamples},'FontWeight', 'bold','FontName','Myriad Pro','FontSize',20,'Color','w')
...
saveas(fig,[analysisParam.figDir filesep 'ViolinPlots-SeparatedConditions-Black-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4)],'svg')
saveas(fig,[analysisParam.figDir filesep 'ViolinPlots-SeparatedConditions-Black-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4)],'fig')
...
saveas(fig,[analysisParam.figDir filesep 'ViolinPlots-SeparatedConditions-Black-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4) '.pdf'],'pdf');
