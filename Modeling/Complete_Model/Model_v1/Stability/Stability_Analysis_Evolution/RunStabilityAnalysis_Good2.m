...
%%
load(['MCMC_results_TimeCourse_ODE_v1_SavingAllParams_Good2.mat'])

%%
nacceptedpar = length(AllAcceptedCosts);

significantchangesdistances = [1,find(-AllAcceptedCosts(2:end) + AllAcceptedCosts(1:(end-1))>0.003),nacceptedpar];


for accepterparameternumber  = 1:length(significantchangesdistances)
accepterparameternumber

mTesrBMPlevel = AllAcceptedmTeSRBMPlevels(significantchangesdistances(accepterparameternumber));%AllAcceptedParameters(nacceptedpar).mTesrBMPlevel;
basalBMPlevel = 0;%AllAcceptedParameters(nacceptedpar).basalBMPlevel;
paramBMPfunc = [1,1,30];%AllAcceptedParameters(nacceptedpar).paramBMPfunc;
parameter = parametersim;
parameter(parfitnumbers) = AllAcceptedParameters(:,significantchangesdistances(accepterparameternumber));%AllAcceptedParameters(nacceptedpar).parammodel;
...
criticalpoints = Allcellstability{end}{BMPii,WNTii};
critpointsaux = [];
...
Allcellstabilityclean{accepterparameternumber} = cellstabilityclean;
AllNtrialsallclean{accepterparameternumber} = Ntrialsallclean;
Allnstablepointsmatrixclean{accepterparameternumber} =nstablepointsmatrixclean;
Allstabilityallclean{accepterparameternumber} = stabilityclean;

% eltime = toc

% subplot(5,6,accepterparameternumber)
pcolor(nstablepointsmatrixclean')
colorbar

end

save('StabilityAnalysis_Real_NON-INTEGERCOEFFITIENTS_v1_SensitivityAllGood2_Parameters_Method4_findcriticalpointsv4_tF1000')
