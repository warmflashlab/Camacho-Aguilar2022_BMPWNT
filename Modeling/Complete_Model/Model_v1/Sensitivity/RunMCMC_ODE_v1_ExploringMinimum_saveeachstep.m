...
jj = 0;
kk=1;
newname = ['MCMC_results_TimeCourse_ODE_v1_ExploringMinimum_',num2str(jj)];
...
if (sum(NewCandidateParameter<0)==0)
    ...
    if rem(ii, 1000000) == 0
        ...
        kk=1;
        MCMCChain = [];
        AllSampledParameters= zeros(length(parfitnumbers),1);
        AllSampledmTeSRBMPlevels= [];
        AllSampledCosts = [];
        AllSampledSimulatedTrajs = zeros(size(LastSimulatedTrajs));
