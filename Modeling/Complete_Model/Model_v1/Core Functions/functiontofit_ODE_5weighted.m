function [res,simulatedEndTrajs] = functiontofit_ODE_5weighted(subparameter,structtopass,weights)
...
%         parametervalueaux = parameter;                      
        ...
        simulatedEndTrajs = simulateexperiments_ODE(subparameter,ExpDataStruct,initcondaux,Istar,nExp,dt,2);
        
%         simulatedEndTrajs(:,25)
...
for jj=1:(nExp-2)
    ...
    if (indmaxv==1)&&(aux(1) == aux(2)) %CDX2
        res4 = [res4,sum((ExpDataStruct(jj).MatrixData - simulatedEndTrajs(:,jj)).^2)];
    ...
    elseif (indmaxv==1)&&(aux(1) == aux(3))
        ...
        if indmaxv==2  %BRA
            res2 = [res2,sum((ExpDataStruct(jj).MatrixData - simulatedEndTrajs(:,jj)).^2)];

        elseif indmaxv==1 %SOX2

            res1 = [res1,sum((ExpDataStruct(jj).MatrixData - simulatedEndTrajs(:,jj)).^2)];

        elseif indmaxv==3 %CDX2

            res3 = [res3,sum((ExpDataStruct(jj).MatrixData - simulatedEndTrajs(:,jj)).^2)];
...
res5 = (ExpDataStruct(nExp-1).MatrixData - simulatedEndTrajs(:,nExp-1)).^2 + (ExpDataStruct(nExp).MatrixData(2) - simulatedEndTrajs(2,nExp)).^2;

% for jj=1:nExp
%     
%     [maxv,indmaxv] = max(ExpDataStruct(jj).MatrixData);
%     if indmaxv==2
%         res2 = [res2,sum((ExpDataStruct(jj).MatrixData - simulatedEndTrajs(:,jj)).^2)];
%         
%     else
...
res = (1/sum(weights))*(weights(1)*sum(res1)/length(res1)+weights(2)*sum(res2)/length(res2)+weights(3)*sum(res3)/length(res3)+weights(4)*sum(res4)/length(res4)+weights(5)*sum(res5)/length(res5));

% res = (1/10)*(3*sum(res2)/length(res2)+7*sum(res1)/length(res1));
...
for jj=1:(nExp-2)
    ...
    if (indmaxv==1)&&(aux(1) == aux(2)) %CDX2
        res4 = [res4,sum((ExpDataStruct(jj).MatrixData +1).^2)];
    elseif (indmaxv==2)&&(aux(2) == aux(3))
    ...
    elseif (indmaxv==1)&&(aux(1) == aux(3))
        ...
        if indmaxv==2  %BRA
            res2 = [res2,sum((ExpDataStruct(jj).MatrixData +1).^2)];

        elseif indmaxv==1 %SOX2

            res1 = [res1,sum((ExpDataStruct(jj).MatrixData +1).^2)];

        elseif indmaxv==3 %CDX2

            res3 = [res3,sum((ExpDataStruct(jj).MatrixData+1).^2)];

        end
    end
    
end
res5 = (ExpDataStruct(nExp-1).MatrixData +1).^2 + (ExpDataStruct(nExp).MatrixData(2) +1).^2;
