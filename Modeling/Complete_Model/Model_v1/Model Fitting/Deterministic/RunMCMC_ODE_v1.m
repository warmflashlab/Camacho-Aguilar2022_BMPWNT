...
save('MCMC_results_TimeCourse_ODE_v1_prueba')
...
NewCandidatesignmasbasalBMPlevel = 0.001*parameters.basalBMPlevel;
NewCandidatesignmasparamBMPfunc = 0.1*parameters.paramBMPfunc;
...
NewCandidateParameter = parameters.parammodel(parfitnumbers);
    ...
    if fitparamsmodel
        for npardraw=parfitnew%[1,3,5]%:nparfit
        NewCandidateParameter(npardraw) = normrnd(NewCandidateParameter(npardraw),NewCandidatesigmas(npardraw));
...
if (sum(NewCandidateParameter<0)==0)
    ...
    [LastCost,LastSimulatedTrajs] = functiontofit_ODE_5weighted(parametersaux,structtopass,paramweights);
