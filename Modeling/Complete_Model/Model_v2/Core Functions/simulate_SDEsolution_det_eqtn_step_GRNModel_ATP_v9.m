function [AllXsol,Alltvsol,finalXsol]=simulate_SDEsolution_det_eqtn_step_GRNModel_ATP_v9(initcondSignals,initcondGRN,tspan,Signals,p,tsol,odeopt,nsimulations,noisecov,BMPlevels,mTeSRBMPlevel,basalBMPlevel)
...
while flag 
    ...
    if ii==1
        ...
        Xs0 = initcondGRN;
% size(Xs0)
% size(initcondSignals)
% size(repmat(initcondSignals',1,nsimulations))
...
Alltvsol = tspan{ii}(1);
    ...
    y0signals = y0(1:5);
    tspan{ii}
    solauxsignals = solution_det_eqtn_ONLYSIGNALS_GRNModel_v9(y0signals,tspan{ii},p,odeopt);
