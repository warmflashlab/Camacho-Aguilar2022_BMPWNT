function [AllXsol,Alltvsol]=simulate_solution_det_eqtn_step_GRNModel_ATP_PRED(initcond,tspan,BMPlevels,mTeSRBMPlevel,basalBMPlevel,Signals,p,tsol,odeopt)
...
while flag 
    ...
    if ii==1
        ...
        y0 = solaux.y(:,end);
        y0(1) = Signals(1,ii);%basalBMPlevel+BMPlevels*mTeSRBMPlevel^(Signals(1,ii)>0); %Media change changes the ligand value to a proportion of the original BMP levels
    ...
    solaux = solution_det_eqtn_GRNModel_v9(y0,tspan{ii},p,odeopt);
    
    AllXsol = [AllXsol,deval(solaux,tspan{ii})];
    Alltvsol = [Alltvsol,tspan{ii}];
